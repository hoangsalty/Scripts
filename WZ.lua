...
window:Toggle("Custom Dungeon", "Enable = Custom Mission, Disable = Auto Mission", JSON.CustomD, function(state)
    JSON.CustomD = state
    Save()
end)

dropdown = window:Dropdown("Mission", "Current Mission: "..JSON.Mission, 
{
    "Crabby Crusade",
    "Dire Problem",
    "Scarecrow Defense",
    ...
    dropdown.Description = "Current Mission: "..mission
    Save()
end)

window:Toggle("Redo", "Redo Mission", JSON.Redo, function(state)
    JSON.Redo = state
    Save()
end)

window:Toggle("Auto Equip", "Auto Equip Better Equipment", JSON.Equip, function(state)
    JSON.Equip = state
    Save()
end)

window:Divider("Auto Sell")

window:Toggle("Enable", "Turn On AutoSell", JSON.Sell, function(state)
    JSON.Sell = state
    Save()
end)
...
window:Toggle("Common", "Sell Common", JSON.Common, function(a)
    JSON.Common = a
    Save()
end)

window:Toggle("Uncommon", "Sell Uncommon", JSON.Uncommon, function(a)
    JSON.Uncommon = a
    Save()
end)

window:Toggle("Rare", "Sell Rare", JSON.Rare, function(a)
    JSON.Rare = a
    Save()
end)

window:Toggle("Epic", "Sell Epic", JSON.Epic, function(a)
    JSON.Epic = a
    Save()
end)
...
window:Toggle("START", "Start Auto Farm", JSON.START, function(state)
    JSON.START = state
    Save()
